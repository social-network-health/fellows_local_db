...
| AC-17 (mirrored data is sourced) | Canonical source: the **2026-04-08 Knack REST-API extraction of the EHF fellows directory** (515 records), archived before EHF's Knack SaaS shut down — all contact data derives from that one extraction. `build/restore_from_knack_scrapefile.py` maps every column to a Knack `field_*` (raw_dump fallback); no contact data introduced beyond the configured Knack source. The provenance travels **in-band**: the build stamps a `provenance` table inside `fellows.db` itself (hop 0 = the Knack extraction incl. the scrapefile's sha256; hop 1 = this build pipeline; no per-build volatile values, so rebuilds stay deterministic) — a downstream importer preserves the hops and appends its own. Column-by-column mapping and recovery paths: [`./data_provenance.md`](./data_provenance.md). | `tests/test_database.py::test_provenance_table_has_two_hops`, `tests/test_database.py::test_provenance_hop0_names_knack_source`, `tests/test_database.py::test_provenance_hop1_has_no_volatile_values`; `tests/test_etl_provenance.py` (hermetic: hop-0 sha over exact parsed bytes, byte-deterministic rebuild, diff-tool tolerance); `build/diff_fellows_db.py` (column-exact fellows-table diff vs `fellows.db.backup.2026-04-08`, via `just db-verify`); [`./data_provenance.md`](./data_provenance.md) (human review) | conformant |
...
The build also stamps an **in-band provenance chain** into the same file (AC-17), so "where did this data ultimately come from?" travels with the DB into the raw `/fellows.db` download, the OPFS copy, and any downstream application that imports it:

```sql
CREATE TABLE provenance (
    hop INTEGER PRIMARY KEY,   -- 0 = ultimate source; importers append hops
    system TEXT NOT NULL,      -- 'EHF Fellows Directory (Knack)' / 'fellows_local_db'
    artifact TEXT,             -- 'knack_api_detail_dump.json' / 'fellows.db'
    artifact_sha256 TEXT,      -- sha of the hop's artifact (NULL for the DB itself)
    acquired_at TEXT,          -- source-vintage date ('2026-04-08'), never build time
    method TEXT,               -- 'Knack REST API extraction' / the ETL script
    note TEXT
);
```

Hops are attested claims by the writing application, not proofs; rows carry no per-build volatile values (no wall-clock, no git sha) so rebuilds stay byte-deterministic and `fellows_db_sha` doesn't churn per code commit. Readers must tolerate the table's absence (pre-provenance OPFS copies persist under the install-only policy). Full design rules: [`./data_provenance.md` § In-band provenance](./data_provenance.md#in-band-provenance-the-provenance-table).

> **Drift from spec form:** the spec contract enforces `slug` uniqueness with a table constraint (`UNIQUE (slug)`); fellows uses a separate `CREATE UNIQUE INDEX`. Functionally equivalent — both enforce the same contract at the SQLite level. The `provenance` table is a fellows extension beyond the contract's commented-out single `source` column (SH-6 lets single-source PNAs omit per-record source; the chain answers the *ultimate*-source question instead) — staged as an upstream PNT contribution in [`../plans/pna_toolkit_provenance_chain_contribution.md`](../plans/pna_toolkit_provenance_chain_contribution.md).
...
**Build pipeline:** `build/restore_from_knack_scrapefile.py` reads the Knack API detail dump (with a fallback read of the list-view `raw_dump` for a few fields), deduplicates slugs, detects grey-diamond placeholder avatars by MD5, and writes all three tables (`fellows`, `fellows_fts`, `provenance`). See [`./data_provenance.md`](./data_provenance.md) for the full data pipeline.
